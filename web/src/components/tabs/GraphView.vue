...
/* eslint-disable */
  import $ from 'jquery'
  import { buildGraph } from '../../lib/lib'
  ...
  function reloadGraph(store, activeRow) {
    ...
    const graph = buildGraph(store, activeRow.vma);
    // console.log(graph)
    ...
    //   $('.graph-view').append(odaTemplate('instruction-graph-error', $.extend(activeRow, {
    ...
    methods: {
      redraw () {
        ...
        if (this.visible) {
          ...
          const parcel = this.$store.getters.parcelByVma(this.selectedAddress);
          if (parcel.is_code) {
            reloadGraph(this.$store, {
            ...
            $('.graph-view svg').remove();
            $('.graph-view div').remove();
            $('.graph-view').append(odaTemplate('instruction-graph-data-section', { vma: this.selectedAddress } ));
          }
        }
      }
    }
