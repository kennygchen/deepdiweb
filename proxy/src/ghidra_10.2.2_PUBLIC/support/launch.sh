...
# GHIDRA_SCRIPT_DIR="../../SigmaDiff/ghidra_script/"
# CLASSPATH="$CLASSPATH:$GHIDRA_SCRIPT_DIR"
# export CLASSPATH

# # Compile all Java files in the specified directory
# javac "${GHIDRA_SCRIPT_DIR}*.java"


# GHIDRA_SCRIPT_DIR="$(pwd)/ghidra_script/"
...
if [ -f "${SUPPORT_DIR}/launch.properties" ]; then
	...
	# GHIDRA_SCRIPT_DIR="${INSTALL_DIR}/../SigmaDiff/ghidra_script"
	GHIDRA_SCRIPT_DIR="${SUPPORT_DIR}/../../SigmaDiff/ghidra_script"

	CPATH="${INSTALL_DIR}/Ghidra/Framework/Utility/lib/Utility.jar:${GHIDRA_SCRIPT_DIR}/json-simple-1.1.1.jar"
	...
	GHIDRA_SCRIPT_DIR="${INSTALL_DIR}/../SigmaDiff/ghidra_script"
	GHIDRA_SCRIPT_DIR="${SUPPORT_DIR}/../../SigmaDiff/ghidra_script"
	# GHIDRA_SCRIPT_DIR="/Users/luyao/Documents/UCR_Course/DirectedResearch/2022_deepbits/sigmadiff/deepdiweb/proxy/src//SigmaDiff/ghidra_script*.java"
	CPATH="${INSTALL_DIR}/Ghidra/Framework/Utility/bin/main:${GHIDRA_SCRIPT_DIR}/json-simple-1.1.1.jar"
...
CLASSPATH="$CLASSPATH:$INSTALL_DIR/Ghidra/Framework/SoftwareModeling/lib/*:$GHIDRA_SCRIPT_DIR"
export CLASSPATH
echo "${GHIDRA_SCRIPT_DIR}"

# Compile all Java files in the specified directory
for file in ${GHIDRA_SCRIPT_DIR}/*.java; do
    javac "$file"
    echo "-------- $file"
    exit 1
done
